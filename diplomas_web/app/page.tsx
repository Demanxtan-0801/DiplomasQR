...
const Icon = {
  ...
  spinner: () => <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="animate-spin"><circle cx="12" cy="12" r="10" strokeOpacity=".25"/><path d="M22 12a10 10 0 0 1-10 10" stroke="currentColor"/></svg>,
  ...
  qr: () => <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="5" y="5" width="3" height="3" fill="currentColor" stroke="none"/><rect x="16" y="5" width="3" height="3" fill="currentColor" stroke="none"/><rect x="5" y="16" width="3" height="3" fill="currentColor" stroke="none"/><path d="M14 14h3v3h-3zM17 17h3v3h-3zM14 17v3"/></svg>,
}
...
function Badge({ children, color = 'blue' }: { children: React.ReactNode; color?: string }) {
  ...
  return (
    <span style={{
      ...Object.fromEntries((colors[color]||colors.blue).split(';').map(s=>s.split(':'))),
...
// ── Upload Zone ────────────────────────────────────────────────────────────
function UploadZone({ onResult }: { onResult: (data: ProcessResult) => void }) {
    ...
    <div>
      <div
        ...
        style={{
          ...
          background: dragging ? 'var(--light)' : 'var(--white)',
          transition: 'all .2s',
        }}
      >
        ...
        {loading ? (
          ...
          <>
            <div style={{
              width:56,height:56,borderRadius:12,background:'var(--light)',
              display:'flex',alignItems:'center',justifyContent:'center',
              margin:'0 auto 16px',color:'var(--accent)'
            }}>
              <Icon.upload />
            </div>
            <p style={{fontWeight:600,color:'var(--navy)',marginBottom:6}}>
      ...
      {error && (
        <div style={{marginTop:10,padding:'10px 14px',background:'#fef2f2',border:'1px solid #fecaca',
          borderRadius:8,color:'var(--error)',fontSize:13,display:'flex',alignItems:'center',gap:8}}>
          <Icon.x />{error}
        </div>
      )}
    </div>
...
function SuccessPanel({ result, onReset }: { result: ProcessResult; onReset: () => void }) {
  return (
    <div className="animate-fade-up" style={{
      background:'var(--white)', borderRadius:12, border:'1px solid var(--border)', overflow:'hidden'
    }}>
      {/* Header */}
      ...
      {/* Info */}
      <div style={{padding:'16px 24px',background:'var(--light)',
        borderBottom:'1px solid var(--border)',display:'flex',gap:24,flexWrap:'wrap'}}>
        <div style={{fontSize:13,color:'var(--muted)',display:'flex',alignItems:'center',gap:6}}>
          <Icon.calendar />
          <span>Período: <strong style={{color:'var(--navy)'}}>{result.fechas_texto}</strong></span>
        </div>
        <div style={{fontSize:13,color:'var(--muted)',display:'flex',alignItems:'center',gap:6}}>
          <Icon.calendar />
          <span>Válido hasta: <strong style={{color:'var(--navy)'}}>{result.valido_hasta}</strong></span>
        </div>
        <div style={{fontSize:13,color:'var(--muted)',display:'flex',alignItems:'center',gap:6}}>
          <Icon.qr />
          <span>QR generado por script Python local</span>
        </div>
      </div>
      ...
      <div style={{padding:'16px 24px',background:'#fffbeb',borderBottom:'1px solid #fde68a'}}>
        ...
        <p style={{fontSize:12,color:'#78350f',lineHeight:1.6}}>
          Los datos se registraron en Supabase. Ahora ejecuta el script Python con el mismo Excel
          para generar los diplomas .docx → PDF y subirlos. El QR de cada diploma apuntará
          al PDF en Supabase.
        </p>
        <code style={{display:'block',marginTop:8,background:'#fef3c7',padding:'6px 10px',
          borderRadius:6,fontSize:12,color:'#78350f'}}>
          python generar_diplomas_supabase.py
        </code>
      </div>
      ...
      <div style={{padding:'14px 24px',borderTop:'1px solid var(--border)',
        ...
        <button onClick={onReset} style={{
          ...
          background:'white',color:'var(--navy)',fontSize:13,fontWeight:600,cursor:'pointer'
        }}>
          Subir otro Excel
        </button>
      </div>
...
// ── Curso Card ─────────────────────────────────────────────────────────────
function CursoCard({ curso, selected, onClick }: {
  curso: Curso; selected: boolean; onClick: () => void
}) {
  return (
    <div onClick={onClick} style={{
      padding:'14px 16px', borderRadius:10, cursor:'pointer',
      border: `2px solid ${selected ? 'var(--accent)' : 'var(--border)'}`,
      background: selected ? 'var(--light)' : 'var(--white)',
      transition:'all .15s',
    }}>
      <div style={{display:'flex',alignItems:'flex-start',gap:10}}>
        ...
        <div style={{minWidth:0}}>
          <p style={{fontWeight:600,fontSize:13,color:'var(--navy)',
            ...
            maxWidth:200}}>{curso.nombre_curso}</p>
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/cursos')
      .then(r => r.json())
      .then(data => { setCursos(data || []); setLoadingCursos(false) })
      .catch(() => setLoadingCursos(false))
  }, [result]) // recarga cuando se sube un nuevo excel

  // Cargar diplomas cuando se selecciona un curso
  useEffect(() => {
    if (!selectedCurso) return
    setLoadingDiplomas(true)
    fetch(`/api/diplomas?curso_id=${selectedCurso.id}`)
      .then(r => r.json())
      .then(data => { setDiplomas(data || []); setLoadingDiplomas(false) })
      .catch(() => setLoadingDiplomas(false))
  }, [selectedCurso])

  function handleResult(data: ProcessResult) {
    setResult(data)
    setSelected(null)
  }
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)'}}>
      ...
      <header style={{
        ...
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            ...
            <p style={{color:'rgba(255,255,255,.5)',fontSize:11,lineHeight:1,marginTop:2}}>
              Gestión de Diplomas
            </p>
          </div>
        </div>
      ...
      <main style={{display:'grid',gridTemplateColumns:'280px 1fr',gap:0,height:'calc(100vh - 60px)'}}>
        ...
        <aside style={{
          ...
          <div style={{padding:'20px 16px 12px',borderBottom:'1px solid var(--border)'}}>
            <p style={{fontWeight:700,fontSize:14,color:'var(--navy)',marginBottom:2}}>
              Cursos registrados
            </p>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:8}}>
                {cursos.map(c => (
                  <CursoCard key={c.id} curso={c}
                    selected={selectedCurso?.id === c.id}
        ...
        <div style={{overflow:'auto',padding:'28px 32px',display:'flex',flexDirection:'column',gap:24}}>
          ...
          {!selectedCurso && !result && cursos.length > 0 && (
            <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',
              color:'var(--muted)',textAlign:'center',gap:12,flexDirection:'column',padding:40}}>
